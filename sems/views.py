...
def paraqit_provimet(request):
    ...
    if request.method == 'GET':
        if request.GET.get('filter'):
            ...
            semester = int(request.GET.get('sessions'))
    ...
    return render (
        request, 'present_exams.html', {'program': program, 'courses': courses, 'provimet': provimet, 'afatet': afatet, 'afatetAll': afatetAll}, 
    )
...
def provimet_paraqitura(request):
    ...
    return render (
        request, 'exams_submitted.html', {'program': program, 'provimet': provimet, 'afatetAll': afatetAll}, 
    )
